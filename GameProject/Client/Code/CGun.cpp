...
#pragma region RayCast를 이용한 크로스헤어 도달점 계산
            ...
            if (pLayer != nullptr)
            {
                ...
                for (auto& pair : mapCurLayer)
                {
                    if (pair.first != L"Player" &&
                        ...
                        pair.first != L"Bullet")
                    {
                        CVIBuffer* pTextureCom = dynamic_cast<CVIBuffer*>(pair.second->Get_Component(ID_STATIC, L"Com_Buffer")); // VIBuffer 포인터 획득
                        if (pTextureCom != nullptr)
                        {
                            CTransform* pTransCom = dynamic_cast<CTransform*>(pair.second->Get_Component(ID_DYNAMIC, L"Com_Transform")); // Transform 포인터 획득
                            if (pTransCom != nullptr)
                            {
                                // RayCast 연산 및 VtxBuffer, IdxBuffer 접근에 필요한 기능구현
                                ...
                                switch (vtxStructType) // 컴포넌트에 맞는 버텍스 구조체 생성후 템플릿함수 대입
                                {
                                case VTXSTRUCT_COL:
                                {
